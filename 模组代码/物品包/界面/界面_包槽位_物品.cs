...
public class 类型_包槽位_物品( 类型_物品包 所属包, int 槽位索引 ) : UIElement {
    ...
    public override void LeftClick( UIMouseEvent 鼠标事件 ) {
        ...
        if ( 鼠标物品.IsAir ) {
            if ( !所属物品.IsAir ) LeftClick_交换物品();
            return; 
        }
        if ( !所属包.放入许可( 鼠标物品 ) ) { SoundEngine.PlaySound( SoundID.MenuClose ); return; }
        if ( 鼠标物品.type == 所属物品.type ) {
            if ( ItemLoader.CanStack( 所属物品, 鼠标物品 ) ) LeftClick_叠加物品();
            else LeftClick_交换物品(); 
            return; 
        }
        if ( !所属包.玩家.条件符合( 鼠标物品 ) ) { SoundEngine.PlaySound( SoundID.MenuClose ); return; }
        LeftClick_交换物品();
    }
    ...
    internal virtual void LeftClick_叠加物品() {
