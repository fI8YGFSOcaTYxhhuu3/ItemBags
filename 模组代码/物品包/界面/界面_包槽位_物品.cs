...
public partial class 类型_包槽位_物品( 接口_物品包 所属包, int 槽位索引 ) : UIElement {
    ...
    protected virtual void 物品存取处理( Item 先前物品, Item 当前物品 ) { }

    private void LeftClick_交换物品() {
        Item 先前物品 = 所属物品;
    ...
    private void LeftClick_快捷转移() {
        if ( 所属物品.IsAir ) return;

        Item 先前物品 = 所属物品;
        Item 剩余物品 = Main.LocalPlayer.GetItem( Main.myPlayer, 先前物品, GetItemSettings.InventoryEntityToPlayerInventorySettings );

        if ( 剩余物品.IsAir || 剩余物品.stack < 先前物品.stack ) {
            所属包.物品矩阵[ 槽位索引 ] = 剩余物品;
            物品存取处理( 先前物品, 剩余物品 );
            SoundEngine.PlaySound( SoundID.Grab );
        }
...
// 常规 TML 成员
public partial class 类型_包槽位_物品 {
    ...
    public override void LeftClick( UIMouseEvent 鼠标事件 ) {
        if ( ItemSlot.ShiftInUse ) { LeftClick_快捷转移(); return; }
