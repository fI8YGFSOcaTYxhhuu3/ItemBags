...
public class 类型_包槽位_嵌套( 接口_嵌套包 所属包, int 槽位索引 ) : 类型_包槽位_缓存( 所属包, 槽位索引 ) {
    ...
    public override void RightClick( UIMouseEvent 鼠标事件 ) { if ( !所属物品.IsAir ) 类型_系统_界面管理.界面管理器.切换窗口( 所属物品.ModItem as 接口_物品包 ); }
    public override void MiddleClick( UIMouseEvent 鼠标事件 ) { if ( 所属物品.ModItem is 接口_缓存包_可切换 缓存包 ) 缓存包.切换启用状态(); }
}