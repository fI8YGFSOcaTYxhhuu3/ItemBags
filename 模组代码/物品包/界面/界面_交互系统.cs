...
// 特征成员
public partial class 类型_系统_界面交互 : ModSystem {
    public static 类型_界面管理器 界面管理器 { get; set; }
    public static UserInterface 界面接口 { get; set; }

    private static void 处理_退出背包界面() { if ( !Main.playerInventory ) 界面管理器.关闭全部(); }
    private static void 处理_容器物品右键() {
...
// TML 重写函数
public partial class 类型_系统_界面交互 {
    public override void Load() {
        界面管理器 = new(); 界面管理器.Activate();
        界面接口 = new(); 界面接口.SetState( 界面管理器 );
    }
    public override void Unload() { 界面管理器 = null; 界面接口 = null; }
    ...
    public override void PostUpdateInput() { 处理_退出背包界面(); 处理_容器物品右键(); 处理_容器物品中键(); }
}
