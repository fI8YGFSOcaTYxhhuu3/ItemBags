...
public partial class 类型_窗口_物品包配置 : 类型_窗口_通用 {
    ...
    protected override void 界面初始化() {
        ...
        UITextPanel<string> 保存按钮 = new( Language.GetTextValue( "Mods.物品包.UI.按钮.保存配置" ) ) { BackgroundColor = new( 73, 180, 73 ) };
        ...
        UITextPanel<string> 恢复按钮 = new( Language.GetTextValue( "Mods.物品包.UI.按钮.恢复全局" ) );
        恢复按钮.Top.Set( -45f, 1f ); 恢复按钮.Left.Set( 115f, 0f ); 恢复按钮.Width.Set( 100f, 0f ); 恢复按钮.Height.Set( 30f, 0f );
        恢复按钮.OnLeftClick += ( evt, el ) => 更新配置( null );
        Append( 恢复按钮 );

        UITextPanel<string> 取消按钮 = new( Language.GetTextValue( "Mods.物品包.UI.按钮.取消" ) ) { BackgroundColor = new( 180, 73, 73 ) };
