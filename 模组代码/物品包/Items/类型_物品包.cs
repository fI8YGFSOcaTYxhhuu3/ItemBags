...
public partial interface 接口_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> : 接口_物品包 where 泛型_配置 : 类型_配置_物品包 where 泛型_玩家 : 类型_玩家_物品包 where 泛型_包槽位 : 类型_包槽位_物品 {
    泛型_配置 独立配置 { get; set; }
    泛型_配置 默认配置 => ModContent.GetInstance<泛型_配置>();
    泛型_配置 配置 => 独立配置 ?? 默认配置;
...
// TML 虚成员
public partial interface 接口_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> {
    接口_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> Clone( 接口_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> 初始副本 ) => Clone_物品包( 初始副本 );
    protected 接口_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> Clone_物品包( 接口_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> 副本 ) {
        副本.物品矩阵 = Array.ConvertAll( 物品矩阵, 物品 => 物品.Clone() );
        if ( 独立配置 != null ) 副本.独立配置 = 独立配置.Clone() as 泛型_配置;
        return 副本;
    }
    ...
    protected void LoadData_物品包( TagCompound 存档标签 ) {
        ...
        if ( 存档标签.TryGet( "独立配置", out TagCompound 配置标签 ) ) { 独立配置 = ( 泛型_配置 ) 配置.Clone(); 独立配置.存档读取( 配置标签 ); }
    }
    ...
    protected void NetReceive_物品包( BinaryReader 网络流 ) {
        ...
        if ( 网络流.ReadBoolean() ) { 独立配置 = ( 泛型_配置 )  配置.Clone() ; 独立配置.网络接收( 网络流 ); }
    }
}
...
public abstract partial class 类型_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> {
    ...
    public override ModItem Clone( Item 初始副本 ) => ( ModItem )  接口.Clone( base.Clone( 初始副本 ) as 接口_物品包<泛型_配置, 泛型_玩家, 泛型_包槽位> ) ;
