...
// 简单重写成员
public partial interface 接口_护甲包 : 接口_物品包<类型_配置_护甲包, 类型_玩家_护甲包, 类型_包槽位_护甲>, 接口_缓存包_列表<类型_护甲组合>, 接口_缓存包_可切换<类型_缓存_护甲包, 类型_配置_护甲包, 类型_玩家_护甲包, 类型_包槽位_护甲>, 接口_网络同步包<结构_同步数据_护甲包> {
    枚举_物品包类型 接口_物品包.类型标识 => 枚举_物品包类型.护甲包;
    bool 接口_物品包.放入许可( Item 物品 ) => 物品.headSlot >= 0 || 物品.bodySlot >= 0 || 物品.legSlot >= 0;
    类型_包槽位_护甲 接口_物品包<类型_配置_护甲包, 类型_玩家_护甲包, 类型_包槽位_护甲>.界面槽位( int 索引 ) => new( this, 索引 );
    结构_同步数据_护甲包 接口_网络同步包<结构_同步数据_护甲包>.同步数据 => new() { ID = ID, 功能配置 = 配置.功能配置, 数据列表 = 缓存数据 };
}
...
public class 类型_护甲包 : 类型_缓存包<类型_缓存_护甲包, 类型_配置_护甲包, 类型_玩家_护甲包, 类型_包槽位_护甲>, 接口_护甲包;