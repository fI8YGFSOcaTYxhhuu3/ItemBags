...
public class 类型_嵌套包 : 类型_缓存包<类型_物品包> {
    ...
    public static Dictionary<枚举_物品包类型, List<类型_物品包>> 缓存字典初始化() {
        return new() {
            [ 枚举_物品包类型.物品包 ] = [],
            [ 枚举_物品包类型.嵌套包 ] = [],
            [ 枚举_物品包类型.饰品包 ] = [],
    ...
    public override bool 放入许可( Item 物品 ) {
        if ( 物品.ModItem is not 类型_物品包 物品包 ) return false;
        if ( 存在自我嵌套( 物品包 ) ) return false;
        return true;
    }

    protected override void 扫描矩阵() {
        foreach ( var 列表 in 缓存字典.Values ) 列表.Clear();
        foreach ( var 物品 in 物品矩阵 ) if ( 物品.ModItem is 类型_物品包 物品包 ) 缓存字典[ 物品包.类型标识 ].Add( 物品包 ); 
    }
    ...
    private bool 存在自我嵌套( 类型_物品包 目标包 ) {
        ...
        目标嵌套包.更新缓存();
        foreach ( var 子嵌套包 in 目标嵌套包.缓存字典[ 枚举_物品包类型.嵌套包 ] ) if ( 存在自我嵌套( 子嵌套包 ) ) return true;

        return false;
    }

    public override ModItem Clone( Item 克隆目标 ) {
        var 克隆实例 = ( 类型_嵌套包 ) base.Clone( 克隆目标 );
        克隆实例.缓存字典 = 缓存字典初始化();
        return 克隆实例;
    }
