...
public class 类型_玩家_饰品包 : 类型_玩家_缓存包_额外缓存<类型_饰品包, Item> {
    ...
    public override void UpdateVisibleAccessories() {
        if ( 缓存列表_额外缓存.Count == 0 ) return;
        if ( !ModContent.GetInstance<类型_配置_饰品包>().应用视觉效果 ) return;
        foreach ( var 饰品 in CollectionsMarshal.AsSpan( 缓存列表_额外缓存 ) ) Player.UpdateVisibleAccessory( 13, 饰品, true );
    }
