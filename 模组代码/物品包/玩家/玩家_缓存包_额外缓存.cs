...
// 特征成员
public abstract partial class 类型_玩家_缓存包_额外缓存<类型_缓存包类型, 类型_额外缓存对象> : 类型_玩家_缓存包<类型_缓存包类型> where 类型_缓存包类型 : 类型_缓存包_列表<类型_额外缓存对象> {
    ...
    protected virtual void 重构缓存_额外缓存() { 缓存列表_额外缓存.Clear(); foreach ( var 缓存包 in CollectionsMarshal.AsSpan( 缓存列表_缓存包 ) ) { 缓存包.更新缓存(); 缓存列表_额外缓存.AddRange( 缓存包.缓存数据 ); } }
}
