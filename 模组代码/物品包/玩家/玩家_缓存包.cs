...
// 特征成员
public abstract partial class 类型_玩家_缓存包 : 类型_玩家_物品包 {
...
// 特征成员
public abstract partial class 类型_玩家_缓存包<接口_缓存包类型> : 类型_玩家_缓存包 where 接口_缓存包类型 : 接口_物品包 {
    public List<接口_缓存包类型> 缓存列表_缓存包 = [];
    ...
    public override bool 缓存变更检测( ModItem 物品 ) {
        ...
        if ( 物品 is 接口_嵌套包 嵌套包 ) {
            ...
            foreach ( var 子嵌套包 in 嵌套包.缓存数据[ ( byte ) 枚举_物品包类型.嵌套包 ] ) if ( 缓存变更检测( 子嵌套包 as ModItem ) ) return true;
        }
        return false;
    }
}
