...
// 特征重写函数
public partial class 类型_玩家_护甲包 : 类型_玩家_缓存包_额外缓存<类型_护甲包, 类型_护甲组合> {
    protected override void 脏标记更新_额外缓存() { base.脏标记更新_额外缓存(); 注册护甲(); }
    public override void OnEnterWorld() { base.OnEnterWorld(); 配置 = ( 类型_配置_护甲包 ) ModContent.GetInstance<类型_配置_护甲包>().Clone(); }
}
